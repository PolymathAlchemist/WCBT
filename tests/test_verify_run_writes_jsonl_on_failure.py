...
def test_verify_run_writes_jsonl_on_failure_missing_file(tmp_path: Path) -> None:
    ...
    # Induce drift: delete a real payload file
    payload_files = [
        p
    ...
    # At least one record should indicate missing
    assert any(r.get("status") == "missing" for r in records)
