...
def test_app_window_refreshes_scheduling_tab_when_scheduling_tab_becomes_active(
    monkeypatch: MonkeyPatch,
) -> None:
    _app()
    ...
    window = AppWindow()
    try:
        scheduling_tab = cast(_FakeSchedulingTab, window.scheduling_tab)
        assert scheduling_tab.refresh_count == 0

        window.tabs.setCurrentIndex(3)

        assert scheduling_tab.refresh_count == 1

        window.tabs.setCurrentIndex(0)
        window.tabs.setCurrentIndex(3)

        assert scheduling_tab.refresh_count == 2
    finally:
        window.close()
